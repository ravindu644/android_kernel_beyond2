...
if [ "$DEVICE" = "S10-5G" ]; then
    echo -e "[+] Building for Korean Variant is Detected..!"
    cp -r "${WDIR}/binaries/${DEVICE}/tzdev" "${WDIR}/drivers/misc/tzdev"
else
    echo -e "[+] Building for Global Variant is Detected..!"
    cp -r "${WDIR}/binaries/addons/tzdev" "${WDIR}/drivers/misc/tzdev"
fi
